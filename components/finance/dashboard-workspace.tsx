    ...
    <main className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/90">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
          ...
          <div className="flex items-center gap-3"><span className="hidden text-sm text-muted-foreground sm:block">Твоето лично Finanzbereich</span><form action="/auth/logout" method="post"><Button variant="outline" size="sm">Изход</Button></form></div>
        </div>
      </header>
      <div className="mx-auto max-w-6xl px-5 py-10">
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.35fr_1fr]">
          <div className="rounded-2xl border border-border bg-accent p-6 text-accent-foreground"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-accent-foreground/70">Препоръчана следваща стъпка</p><h2 className="mt-2 text-2xl font-bold">Попълни финансовия си профил</h2><p className="mt-3 max-w-lg text-sm leading-6 text-accent-foreground/75">Повече информация помага за по-точна проверка. Започни с доходи, домакинство и постоянни разходи.</p></div><CircleDollarSign className="hidden h-10 w-10 text-primary sm:block" /></div><Button asChild className="mt-6 bg-primary text-primary-foreground hover:bg-primary/90"><Link href="/profil">Отвори профила <ArrowUpRight className="ml-2 h-4 w-4" /></Link></Button></div>
          <div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center justify-between"><div><h2 className="font-semibold text-foreground">Отворени напомняния</h2><p className="mt-1 text-xs text-muted-foreground">Само задачи, които си създал или потвърдил.</p></div><Bell className="h-5 w-5 text-muted-foreground" /></div>{reminders.length ? <ul className="mt-4 divide-y divide-border">{reminders.slice(0, 3).map((item) => <li key={item.id} className="flex items-center gap-3 py-3 text-sm"><CheckCircle2 className="h-4 w-4 shrink-0 text-success" /><span className="flex-1 text-foreground">{item.title}</span><span className="text-xs text-muted-foreground">{item.due_at ? new Date(item.due_at).toLocaleDateString("bg-BG") : "Без срок"}</span></li>)}</ul> : <p className="mt-4 text-sm leading-6 text-muted-foreground">Все още няма отворени задачи.</p>}<Button variant="ghost" size="sm" className="mt-2 px-0 text-primary hover:bg-transparent hover:text-primary" onClick={addReminder} disabled={saving}>{saving ? "Запазване …" : "Добави напомняне"}</Button></div>
        </section>
      </div>
    </main>
  )
