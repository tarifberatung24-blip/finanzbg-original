...
export default function SignUpPage() {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-background px-4 py-12"><div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-sm"><Link href="/" className="text-sm font-semibold text-primary">FinanzberaterBG</Link><h1 className="mt-8 text-3xl font-bold text-foreground">Konto erstellen</h1><p className="mt-2 text-muted-foreground">Starte mit einem klaren Blick auf deine Finanzen.</p>{!otpSent ? <form onSubmit={sendOtp} className="mt-8 space-y-4"><div className="space-y-2"><Label htmlFor="phone">Deutsche Mobilnummer</Label><div className="flex gap-2"><span className="flex h-10 items-center rounded-md border border-input bg-muted px-3 text-sm" aria-hidden="true">+49</span><Input id="phone" type="tel" inputMode="tel" autoComplete="tel" placeholder="151 12345678" required value={phone} onChange={(e) => setPhone(e.target.value)} /></div></div>{smsError && <p role="alert" className="text-sm text-destructive">{smsError}</p>}<Button className="w-full" disabled={loading}>{loading ? "Wird gesendet…" : "Регистрирай се със SMS код"}</Button></form> : <form onSubmit={verifyOtp} className="mt-8 space-y-4"><div className="space-y-2"><Label htmlFor="otp">Въведете кода от SMS</Label><Input id="otp" type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} placeholder="123456" required value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} /></div>{smsError && <p role="alert" className="text-sm text-destructive">{smsError}</p>}<Button className="w-full" disabled={loading}>{loading ? "Проверка…" : "Потвърди регистрацията"}</Button><Button type="button" variant="ghost" className="w-full" disabled={loading || cooldown > 0} onClick={() => { setOtpSent(false); setOtp(""); setSmsError(null) }}>{cooldown > 0 ? `Нов код след ${cooldown} сек.` : "Изпрати нов код"}</Button></form>}<div className="my-6 flex items-center gap-3 text-xs text-muted-foreground"><span className="h-px flex-1 bg-border" />oder mit E-Mail<span className="h-px flex-1 bg-border" /></div><form onSubmit={submit} className="space-y-5"><div className="space-y-2"><Label htmlFor="name">Vorname</Label><Input id="name" required value={name} onChange={(e) => setName(e.target.value)} /></div><div className="space-y-2"><Label htmlFor="email">E-Mail</Label><Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} /></div><div className="space-y-2"><Label htmlFor="password">Passwort</Label><Input id="password" type="password" minLength={6} required value={password} onChange={(e) => setPassword(e.target.value)} /></div>{emailError && <p role="alert" className="text-sm text-destructive">{emailError}</p>}<Button variant="secondary" className="w-full" disabled={loading}>{loading ? "Wird erstellt…" : "Konto mit E-Mail erstellen"}</Button></form><p className="mt-6 text-center text-sm text-muted-foreground">Schon registriert? <Link href="/auth/login" className="font-medium text-primary">Anmelden</Link></p></div></main>
}
