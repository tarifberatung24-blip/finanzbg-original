...
export default async function DashboardPage() {
  ...
  const [profileResult, contractsResult, documentsResult, reviewDocumentsResult, remindersResult, auditResult] = await Promise.all([
    ...
    supabase.from("contracts").select("id,title,category,provider_name,monthly_amount,status,created_at,end_date").eq("household_id", householdId).order("created_at", { ascending: false }).limit(100),
